...
def grep_file(filename: str, pattern1: str, pattern2: str) -> bool:
    ...
    # allow "filename" and "..\src\filename" to exist, older project had
    #  tb.v in same directory as top.v not they are split in src/** and test/**
    dotdot_src_filename = os.path.join("..", "src", filename)
    if not os.path.isfile(filename) and os.path.isfile(dotdot_src_filename):
        filename = dotdot_src_filename
